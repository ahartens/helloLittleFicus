...
public class KmeansPrototype{
    ...
    */
    ...
    ///CONSTRUCTOR METHODS
    public KmeansPrototype(Matrix mat, int kval, DistComputable d){
        setData(mat,kval);
        this.distCalc = d;
    }
    ...
    */
    public KmeansPrototype(Matrix mat, int kval){
        setData(mat,kval);
    }
        ...
        for (int i=0; i<this.cs.length; i++){
            ...
            if (this.cs[i] == 0 ){
                ...
                for (int j=0; j<this.cp[0].length; j++){
                    System.out.printf("idx %d before : %f",i,this.cp[i][j]);
                    this.cp[i][j] = this.cp[idxMaxSSE][j];
                //	System.out.printf("    idx: %d   after : %f and of other : %f\n",idxMaxSSE,this.cp[i][j],this.cp[idxMaxSSE][j]);

                }

            //	System.out.printf("\n");
            //	System.out.println(idxMaxSSE +" has this the greatest SSE "+maxSSE);
                repeat = 1;
            }
        }
    ...
    */
    private void calcClusterMean(){
        ...
        System.out.printf("about to average clusters\n");
        for (int i=0; i<this.k; i++) {
            for (int j=0; j<this.m.getColumnSize(); j++) {
